...
void EventSystem::Tick(World* world, ECS::DefaultTickData data)
{
	...
	if(info->m_VisitedRooms.back() == 6 && m_CurrentEventIndex != 4)
	{
		if(std::find(info->m_CollidingEntities.begin(), info->m_CollidingEntities.end(), "room_06_triggercoll_01") != info->m_CollidingEntities.end() && info->m_LookingEntity == "room_03_door_01")
		{
